...
function withinUpperOrLowerCaseRange(charCode: number, base: CharType): boolean {
...
function convertCharCodeToRot13(charCode: number, base: CharType): number {
    let zeroBasedCharCode = charCode - base;
    let shiftedCharCode = zeroBasedCharCode + ROT13_SHIFT;
    let wrappedAround = shiftedCharCode % ALPHA_LENGTH;
    let result = wrappedAround + base;
    return result;
};