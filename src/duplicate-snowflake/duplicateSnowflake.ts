...
export function checkDuplicateInPlace(sf1: Snowflake, sf2: Snowflake, sf2Offset: number = 0) {
    for (let i = 0; i<sf1.points.length; i++) {
        ...
        if (sf1.points[i] !== sf2.points[(i+sf2Offset) % (sf2.points.length-1)]) {
            return false;
        }
    }
    return true;
...
export function checkDuplicateReversed(sf1: Snowflake, sf2: Snowflake) {
    ...
    for (let i = 0; i<sf1.points.length; i++) {
        if (sf1.points[i] !== sf2.points[endOfSf2-i])
            return false;
    }
    return true;
}
